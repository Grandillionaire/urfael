# Security — read this before you run Urfael

Urfael runs an **LLM agent (Claude Code) with access to your shell, files, network, and — if you
...
This is an open, unsolved problem for all tool-using agents. Urfael mitigates it (untrusted-data
...
- **`bypassPermissions` is OFF by default.** The agent runs in `acceptEdits` mode (auto-applies file
  edits; other risky tools are gated). Full unrestricted shell is **opt-in** via `URFAEL_YOLO=1`.
...
## If you enable full power (`URFAEL_YOLO=1`) — do this
...
4. Watch the HUD / `~/.claude/urfael/urfael.log`. A `WARN … URFAEL_YOLO active` line is logged on start.
...
- **Owner-allowlisted.** Each bridge answers a **single** chat/user id (set in the gitignored
  `~/.claude/urfael/bridge.env`). Every other sender is dropped *before* anything reaches the brain.
...
- **Structurally non-bypass.** Every remote turn is tagged with its `channel`, which the daemon maps to
  ...
  `bypassPermissions` even when `URFAEL_YOLO=1`**, runs `--strict-mcp-config` (no browser/desktop/vision
...
- **Untrusted-data framing + audit + rate limit.** Inbound text is wrapped in an untrusted-data envelope
  (prompt-injection mitigation), every turn is appended to `~/.claude/urfael/bridge-audit.log`, and a
...
Net: a remote message can ask Urfael to read and search your vault — it **cannot** write files, run any
...
**By default Urfael uses no API keys at all** — voice is fully local (macOS `say` + whisper.cpp), so a
...
- `~/.claude/urfael/tts.env` — only if you add ElevenLabs (voice) or Picovoice (wake word) keys
- `~/.claude/urfael/api-keys.env` — Tavily / news / finance keys
- `~/.claude/urfael/bridge.env` — only if you enable a Telegram/Discord bridge (bot token + your owner id)

...
- Frozen: a 10th attack-class with a transport probe (`npm run security`, now 10/10 classes, 85/85 checks) plus the broker (14 tests) and brokerd (9 tests) unit suites and the fuzz targets, so none of it can silently regress.

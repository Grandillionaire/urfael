...
The curated registry ships in [config/providers.json](https://github.com/Grandillionaire/urfael/blob/main/config/providers.json): Claude (native, Bedrock, Vertex), OpenRouter, DeepSeek, Ollama, LM Studio, vLLM, claude-code-router, and LiteLLM. Point `URFAEL_PROVIDERS_INDEX` at your own JSON (same schema) to replace it.

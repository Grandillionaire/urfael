...
// The competitor radar is an OWNER-ONLY tool (the founder's "what did Hermes/OpenClaw ship" watch). It is OFF by
// default and entirely invisible — no dashboard section, no endpoints, no scan — unless the owner explicitly opts in
// with URFAEL_RADAR=1 (or URFAEL_RADAR_DAYS=N to also schedule). A downloaded copy never sees it.
const RADAR_ENABLED = /^(1|on|true)$/i.test(process.env.URFAEL_RADAR || '') || (parseInt(process.env.URFAEL_RADAR_DAYS, 10) || 0) > 0;
...
function listen() {
  ...
  server.listen(SOCK, () => {
    ...
    // competitor radar (opt-in URFAEL_RADAR_DAYS): every N days, scan rival releases and notify the owner if a new
    // one was analyzed. It only writes a REPORT to review; it never implements or ships anything on its own.
    { const days = parseInt(process.env.URFAEL_RADAR_DAYS, 10) || 0;
      if (days > 0) {

...
cd app && npm install
npm test             # 994 unit tests (fast, pure modules: lib, council, personas, seal, audit-chain, registry, recall, …); no credentials, ~0.5s
...
2. `npm test` (994 unit tests) and `npm run security` (116/116 checks, 11/11 attack classes) pass, and the app still launches.

...
cd app && npm install
npm test             # 377 fast unit tests (pure modules: lib, council, personas, seal, audit-chain, registry, recall, …); no credentials, ~0.5s
npm run security     # 10/10 attack classes · 87/87 checks — boots a REAL daemon + dashboard and attacks them
...
2. `npm test` (377) and `npm run security` (10/10 · 87/87) pass, and the app still launches.

...
cd app && npm install
npm test             # 906 unit tests (fast, pure modules: lib, council, personas, seal, audit-chain, registry, recall, …); no credentials, ~0.5s
npm run security     # 11/11 attack classes · 108/108 checks — boots a REAL daemon + dashboard and attacks them
...
2. `npm test` (906 unit tests) and `npm run security` (108/108 checks, 11/11 attack classes) pass, and the app still launches.

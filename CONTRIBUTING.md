...
cd app && npm install
npm test             # 801 unit tests (fast, pure modules: lib, council, personas, seal, audit-chain, registry, recall, …); no credentials, ~0.5s
npm run security     # 10/10 attack classes · 101/101 checks — boots a REAL daemon + dashboard and attacks them
...
2. `npm test` (801 unit tests) and `npm run security` (101/101 checks, 10/10 attack classes) pass, and the app still launches.

...
cd app && npm install
npm test             # 939 unit tests (fast, pure modules: lib, council, personas, seal, audit-chain, registry, recall, …); no credentials, ~0.5s
npm run security     # 11/11 attack classes · 111/111 checks — boots a REAL daemon + dashboard and attacks them
...
2. `npm test` (939 unit tests) and `npm run security` (111/111 checks, 11/11 attack classes) pass, and the app still launches.

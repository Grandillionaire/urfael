...
cd app && npm install
npm test             # 335 fast unit tests (pure modules: lib, council, personas, seal, audit-chain, registry, recall, …); no credentials, ~0.5s
npm run security     # 10/10 attack classes · 84/84 checks — boots a REAL daemon + dashboard and attacks them
...
2. `npm test` (335) and `npm run security` (10/10 · 84/84) pass, and the app still launches.

...
cd app && npm install
npm test             # 1158 unit tests (fast, pure modules: lib, council, personas, seal, audit-chain, registry, recall, …); no credentials, ~0.5s
npm run security     # 11/11 attack classes · 121/121 checks — boots a REAL daemon + dashboard and attacks them
...
2. `npm test` (1158 unit tests) and `npm run security` (121/121 checks, 11/11 attack classes) pass, and the app still launches.

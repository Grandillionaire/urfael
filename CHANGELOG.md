...
- **`urfael radar`, an autonomous, human-gated competitor watch.** It watches the rival self-hosted agents (Hermes and OpenClaw) for new releases and, when they ship, runs an honest analysis through your own `claude` CLI: what they shipped, whether Urfael already does it (often better, citing real files), whether it fits Urfael's principles, and how to do it in our zero-dependency style. It writes a report to `~/.claude/urfael/radar/` for you to review and approve; it never implements or ships anything on its own. Run it with `urfael radar`, or schedule it with `URFAEL_RADAR_DAYS=N`.

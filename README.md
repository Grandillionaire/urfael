...
<img src="docs/media/demo.gif" width="820" alt="Urfael resisting the attack classes that compromised other agents: npm run security boots the real daemon and prints 11/11 attack classes, 119/119 checks passed" />

<sub><code>npm run security</code> boots the real daemon and runs the actual attack classes that owned other agents in the wild. It lands on 11/11 classes, 119/119 checks. The proof is a command you run, not a claim you read.</sub>
...
- **The Familiar** (`URFAEL_PET=1`, opt-in, cosmetic): a small **code-drawn** companion whose pose tracks the agent's state (idle / thinking / tool, wielding the real implement rune / waiting / failed), plus the voice-native listening / speaking the orb already earns. One pure module drives both surfaces: a vector layer on the orb HUD and a Unicode glyph that prefixes the TUI worker row, with a 7-bit ASCII fallback and a static reduce-motion pose. Zero token/cache impact **by construction** (the module has no imports and cannot reach the daemon, its socket, the transcript, or the cost ledger; a source-scan test freezes that), no new dependency, no new port, drawn in code (never a sprite atlas). OFF by default, so every default render path is byte-identical; the orb familiar needs `URFAEL_ORB` on too.

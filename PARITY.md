...
| Active recall (proactive per-turn) | agent must invoke semantic search | frozen MEMORY.md+USER.md snapshot injected once at session start (never updated mid-session) + agent-invoked `session_search` (FTS5) | ✦ **retrieval on every owner turn**: the message itself queries the whole-archive index for the most relevant past turns + the trusted lessons that bear on it, injected as a **bounded, fenced** "recalled memory" block (reference-not-instructions, so a once-poisoned turn can't hijack a later one) and **reinforced** in the ledger when surfaced (the testing effect). Neither competitor retrieves per turn; both wait for the agent to search. Hot path is fast BM25 (semantic stays on the on-demand `/recall`); `URFAEL_ACTIVE_RECALL=0` to disable |

...
- **Genuinely ahead (the 12 wins):** the security posture (no inbound port, fail-closed roster, role-scoped
  sandboxes, SSRF-filtered relay, **87/87 benchmark across 10 attack classes**), self-verifying learning (quality
...
| Connectors / integrations | ClawHub plugins `install <src>:<pkg>` (plaintext config, **restart required**, no preview) | `hermes mcp` + 300+ tools (plaintext config/env, **restart required**) | ✦ `urfael connect` — connectors **are MCP servers**, so the whole **18,000+ server ecosystem** is reachable; the popular ones are one command with a **pre-enable security preview + static scan**, a **masked secret prompt** (passed as an `execFile` argv → never in `~/.zsh_history`), **no restart**, and **owner-turns-only** (`--strict-mcp-config` on every sandbox). The setup the rest of the field doesn't ship; frozen as a benchmark check (87/87) |
...
*All of the above are net-new in June 2026, unit- + benchmark-guarded (87/87, incl. a "correctness & craft regressions" class that fails the build if did-you-mean, the doctor's writability check, the real sparkline, or the provenance SHA ever regress). Scored honestly: did-you-mean is parity (table-stakes), the rest are genuine differentiation because the felt experience — not the raw capability — is the point.*

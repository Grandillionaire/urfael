...
and returns data with no daemon, no socket, no `claude`. That is why 1117 unit tests run in ~0.5s with zero
...
| `app/test/` | 1117 `node:test` unit tests (`*.test.js`, pure, no creds) + two live-daemon harnesses (`e2e.js`, `security-benchmark.js`) run only via their own npm scripts. |
...
- **1117 fast unit tests** (`npm test` → `node --test test/*.test.js`) — pure modules, no credentials, ~0.5s.

...
boots the real daemon and dashboard and attacks them, printing **11/11 attack classes · 121/121 checks**.
...
and returns data with no daemon, no socket, no `claude`. That is why 1158 unit tests run in ~0.5s with zero
...
| `app/test/` | 1158 `node:test` unit tests (`*.test.js`, pure, no creds) + two live-daemon harnesses (`e2e.js`, `security-benchmark.js`) run only via their own npm scripts. |
...
- **1158 fast unit tests** (`npm test` → `node --test test/*.test.js`) — pure modules, no credentials, ~0.5s.
...
- **The security benchmark** (`npm run security` → `node test/security-benchmark.js`) — boots the **real**
  ...
  **11/11 attack classes · 121/121 individual checks**. Class 9, "correctness & craft regressions," guards against

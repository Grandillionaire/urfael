...
boots the real daemon and dashboard and attacks them, printing **10/10 attack classes · 90/90 checks**.
...
and returns data with no daemon, no socket, no `claude`. That is why 399 unit tests run in ~0.5s with zero
...
- **The security benchmark** (`npm run security` → `node test/security-benchmark.js`) — boots the **real**
  ...
  **10/10 attack classes · 90/90 individual checks**. Class 9, "correctness & craft regressions," guards against

...
boots the real daemon and dashboard and attacks them, printing **10/10 attack classes · 101/101 checks**.
...
and returns data with no daemon, no socket, no `claude`. That is why 966 unit tests run in ~0.5s with zero
...
| `app/test/` | 917 `node:test` unit tests (`*.test.js`, pure, no creds) + two live-daemon harnesses (`e2e.js`, `security-benchmark.js`) run only via their own npm scripts. |
...
- **917 fast unit tests** (`npm test` → `node --test test/*.test.js`) — pure modules, no credentials, ~0.5s.
...
- **The security benchmark** (`npm run security` → `node test/security-benchmark.js`) — boots the **real**
  ...
  **10/10 attack classes · 101/101 individual checks**. Class 9, "correctness & craft regressions," guards against

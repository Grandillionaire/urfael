...
<<<<<<< HEAD
and returns data with no daemon, no socket, no `claude`. That is why 1061 unit tests run in ~0.5s with zero
=======
and returns data with no daemon, no socket, no `claude`. That is why 1069 unit tests run in ~0.5s with zero
...
<<<<<<< HEAD
| `app/test/` | 1061 `node:test` unit tests (`*.test.js`, pure, no creds) + two live-daemon harnesses (`e2e.js`, `security-benchmark.js`) run only via their own npm scripts. |
=======
| `app/test/` | 1069 `node:test` unit tests (`*.test.js`, pure, no creds) + two live-daemon harnesses (`e2e.js`, `security-benchmark.js`) run only via their own npm scripts. |
>>>>>>> feat/frontier-sleeptime
...
<<<<<<< HEAD
- **1061 fast unit tests** (`npm test` → `node --test test/*.test.js`) — pure modules, no credentials, ~0.5s.
=======
- **1069 fast unit tests** (`npm test` → `node --test test/*.test.js`) — pure modules, no credentials, ~0.5s.
>>>>>>> feat/frontier-sleeptime

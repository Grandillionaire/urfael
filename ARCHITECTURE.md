...
boots the real daemon and dashboard and attacks them, printing **9/9 attack classes · 76/76 checks**.
...
and returns data with no daemon, no socket, no `claude`. That is why 305 unit tests run in ~0.5s with zero
...
- **305 fast unit tests** (`npm test` → `node --test test/*.test.js`) — pure modules, no credentials, ~0.5s.
...
- **The security benchmark** (`npm run security` → `node test/security-benchmark.js`) — boots the **real**
  ...
  **9/9 attack classes · 76/76 individual checks**. Class 9, "correctness & craft regressions," guards against
